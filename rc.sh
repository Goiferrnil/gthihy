...
function checkSystemForUpdate() {
    ...
    elif grep </etc/issue -q -i "gentoo" && [[ -f "/etc/issue" ]] || grep </proc/version -q -i "gentoo"; then
        release="gentoo"
        installType='emerge -q'
        upgrade="emerge -avuDN @world"
        removeType='emerge --depclean -q'
	...
	if ! [ -x "$(command -v dig)" ]; then
        ...
        elif [[ ${release} == "gentoo" ]]; then  # 添加一个新的条件判断
            ${installType} "bind-tools"  # 安装 "bind-tools"（Gentoo系统中的DNS工具）
		else
